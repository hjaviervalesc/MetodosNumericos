...
@ti.kernel
#Cuanto y dónde se rompe la incompresibilidad del fluido, es decir, cuando no es 0 y por lo tanto se necesita corregir.
...
@ti.kernel
#Resuelve la ecuación de Poisson para encontrar el campo de presión que corrige la divergencia.
...
@ti.kernel
#Corrige el campo de velocidad restando el gradiente del campo de presión, asegurando que el campo de velocidad resultante sea incompresible.
...
def init():
    ...
    vel_v.cur.fill(0) 
    vel_v.nxt.fill(0)
    #inicializo divergencia y presión
    div.fill(0)
    p.cur.fill(0)
    p.nxt.fill(0)
